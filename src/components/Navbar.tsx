...
export function getHeaderClassName(isScrolled: boolean, isMenuOpen: boolean): string {
  return [
    ...
    // Keep the stable logo/toggle layer above the portaled overlay, while the empty parts of
    // the header remain transparent to pointer input. The two real controls opt back in below.
...
export function getHeaderInnerClassName(_isMenuOpen: boolean): string {
  return "container-wide flex h-[var(--header-height)] items-center gap-4";
}
...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className={getHeaderInnerClassName(isMenuOpen)}>
        <NavLink
          to="/"
          aria-label="Horizon Digital home"
          title="Home"
          className="site-header-brand focus-ring pointer-events-auto inline-flex min-h-11 items-center rounded-xl"
          onClick={() => {
            setIsMenuOpen(false);
        ...
        <div className="ml-auto flex items-center gap-3 xl:gap-5">
          ...
          <NavLink
            ...
            className="consultation-attraction focus-ring hidden min-h-11 items-center justify-center rounded-xl bg-accent px-5 text-center text-[0.68rem] font-black uppercase tracking-[0.14em] text-[#071013] transition-[background-color,transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:bg-accent-strong hover:shadow-[0_10px_28px_rgba(88,213,227,0.18)] active:translate-y-0 xl:inline-flex 2xl:px-6"
            onClick={() =>
